...
public class Game implements ObservadoIF {
	...
	private Game() {
		...
		getByColor = ColorRules.getInstance();
	}
	...
	private void NextPlayer() {
		...
		SetDiceValue(0);
		SetCurrentState(0);
		GameFacade.getInstance().SetRollDiceState();
	}
	...
	private boolean TryMoving(int[] pieceSelected) {
		...
		//START
		if(decodedPiece == -1) {
			if(!AreThereTwoPiecesInStartHouse(color) && GetDiceValue() == valueNeededToLeaveStartPlace) {
	...
	private boolean CanMoveAutomatically(String color) {
		return AreAllPiecesInStart(color) && !AreThereTwoPiecesInStartHouse(color) && GetDiceValue() == valueNeededToLeaveStartPlace;
	}
	...
	private boolean CanMove(String color) {
		if(IsThereAPieceInStart(color) && GetDiceValue() == valueNeededToLeaveStartPlace &&	!AreThereTwoPiecesInStartHouse(color)) {
	...
	private boolean IsInShelter(int piece) {
		return piece == 52 || piece == 13 || piece == 26 || piece == 39 || piece == 4|| piece == 17 || piece == 30 || piece == 43;
	}
	...
	public Object[] updateGame(String message) {
		String[] updatedGame = message.split(" ");
		this.SetCurrentPlayer(Integer.parseInt(updatedGame[2]));
		this.SetCurrentState(Integer.parseInt(updatedGame[3]));
		this.SetWaitingForPlayer(Integer.parseInt(updatedGame[4]) == 1 ? true : false);
		this.SetDiceValue(Integer.parseInt(updatedGame[5]));
		int[][][] updatedPieces = new int[4][4][2];
		int i = 6;
		for(int j = 0; j < 4; j++){
			for(int k = 0; k < 4; k++) {
				for(int l = 0; l < 2; l++) {
					updatedPieces[j][k][l] = Integer.parseInt(updatedGame[i]);
					if(i < updatedGame.length) i++;
				}
			}
		}
		piecesclass.SetAll(updatedPieces);
		return new Object[]{this, piecesclass};
	}
}